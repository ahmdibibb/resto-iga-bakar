...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create Midtrans Snap transaction if payment method is QRIS
    let snapData: { token: string; redirectUrl: string; gatewayOrderId: string } | null = null
    if (payment_method === 'QRIS') {
      ...
      const itemDetails = orderItems.map((item, index) => ({
        id: items[index].productId as string,
        price: Math.round(item.price.toNumber() * item.quantity / item.quantity),
        quantity: item.quantity,
        name: (order.items[index]?.product?.name ?? `Item ${index + 1}`).substring(0, 50),
      }))

      // Verify item totals match gross_amount
      ...
      if (itemTotal !== grossAmount) {
        // Add adjustment item to reconcile rounding
        itemDetails.push({
          id: 'adjustment',
      ...
      const snapResponse = await createSnapTransaction({
        ...
        grossAmount,
        customerDetails: {
          first_name: (customerName?.trim() || 'Pelanggan').substring(0, 50),
