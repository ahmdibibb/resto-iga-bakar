...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    // If redirected back from Midtrans (URL contains status_code or transaction_status)
    if (typeof window !== 'undefined') {
      const urlParams = new URLSearchParams(window.location.search)
      if (urlParams.has('transaction_status') || urlParams.has('status_code')) {
  ...
  const handleOpenSnap = useCallback(() => {
    if (!order?.payment?.qris_string) return
    if (!snapReady || typeof window === 'undefined' || !window.snap) {
      setError({ message: 'Midtrans Snap belum siap. Mohon tunggu sebentar.', type: 'validation' })
      return
    }
    ...
    window.snap.pay(snapToken, {
      ...
      onError() {
        setError({ message: 'Pembayaran gagal. Silakan coba lagi.', type: 'validation' })
        fetchOrder()
      },
      onClose() {
        handleConfirmPayment()
  ...
  }, [order?.payment?.qris_string, snapReady, handleConfirmPayment])
  ...
  }, [order?.payment_method, order?.payment_status, snapReady, handleOpenSnap])
  ...
  const getStatusBanner = () => {
    ...
    if (order?.payment_method === 'QRIS') {
      return {
        ...
        desc: 'Klik tombol "Bayar dengan QRIS" untuk membuka halaman pembayaran. Pembayaran akan otomatis terverifikasi.',
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            ...
            {/* QRIS Midtrans Snap — only when pending payment */}
            {order.payment_method === 'QRIS' && isPendingPayment && order.payment?.qris_string && (
              <div className="rounded-2xl bg-canvas p-6 border border-hairline shadow-none">
                <h2 className="text-lg font-bold font-jakarta uppercase tracking-tight text-ink mb-4 text-center">Pembayaran QRIS</h2>
                ...
                {order.payment.expires_at && (
                  <p className="text-xs text-center text-amber-600 font-semibold mb-4">
                    <Clock size={12} className="inline mr-1" />Berlaku hingga: {new Date(order.payment.expires_at).toLocaleTimeString('id-ID')}
                    <br />
